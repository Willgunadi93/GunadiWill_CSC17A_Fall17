...
//Function Prototypes
void intro();
void restart(int[][column], int, brdSet);
void myBoard(int[][column], int, brdSet);
bool guess(int[][column], int x, int y);
void insrtion();
void perfmnce(brdSet);
void opFile(ofstream&, brdSet);
...
int main(int argc, char** argv) {
    ...
    int readScr[] = {1,2,1,0,1,2,1,10};    //Max ship/Ship counter
    int ckScore;
    int num1, num2;
    ...
    cin.getline(name,30);
    setting.name = name;
    cout << "Hello " << playerOne.name << "!" << endl;
    cout << "This program simulates a battleship game!" << endl;
    insrtion();
    cout << "---------------------" << endl;
    
    do{
    restart(board, row, setting);
    myBoard(board, row, setting);   
    setting.shpCtr = 10;
    setting.shpDwn = 0;
    
    while (setting.shpDwn < setting.maxShp)           //Set the ship's location
    {
        ...
        {
            setting.shpDwn++;              //Increase ship until reaches maxShip
            board[x][y] = 1;       
        }
    }
    //Initiate loop
    while(setting.shpCtr != 0)       //Play again or not
    {                   
    ...
    cin >> num2;
        ...
        if(guess(board, num1, num2))
        {
            cout << "Hit!" << endl;
            setting.shpCtr--;
            setting.hits++;         //counter for # of hits
            setting.guesses++;      //counter for # of guesses
            cout << "number of ships left " << setting.shpCtr << endl;
            if ( setting.shpCtr == 0)
            {
                cout << "Congratulation!!!\n";
        ...
        {       
            ...
            setting.misses++;       //counter for # of misses
            setting.guesses++;
            cout << "Number of ships left: " << setting.shpCtr << endl;
            if (setting.guesses % 2 == 0)
            {
    ...
    cout << "----------------------------------------" << endl;
    myBoard(board, row, setting);              //Show board
    //Output stats
    perfmnce(setting);
...
void restart(int board[][column], int row, brdSet setting){
    for (int a = 0; a < row; a++)
    {
...
void myBoard(int board[][column],int row, brdSet setting)
...
void perfmnce(brdSet setting)
{
    //structure
    
    cout << "Time to grade your performance.." << endl;
    cout << endl << "number of guesses: " << setting.guesses << endl;
    cout << "number of hits: " << setting.hits << endl;
    cout << "number of misses: " << setting.misses << endl;
    cout << "Percentage of hits: "
         << static_cast<float>(setting.hits)/setting.guesses*100 << "%" << endl;
    cout << "Percentage of misses: " 
         << static_cast<float>(setting.misses)/setting.guesses*100 << "%" << endl;
    switch(setting.hits)
    {
...
void opFile(ofstream& outData, brdSet setting)
{
    ...
    outData << endl << "number of guesses: " << setting.guesses << endl;
    outData << "number of hits: " << setting.hits << endl;
    outData << "number of misses: " << setting.misses << endl;
